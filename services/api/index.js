...
// request logger (RID + method + url + short body)
app.use((req, _res, next) => {
  req._rid = crypto.randomUUID();
  const bodyPreview = req.method !== "GET" ? (JSON.stringify(req.body) || "").slice(0, 500) : "";
  console.log(`[REQ ${req._rid}] ${req.method} ${req.url} ${bodyPreview}`);
  next();
});

// whoami (entry verification)
...
app.use("/users", usersRouter(auth));          // includes admin DELETE/PATCH
...
// error handler with pg details in non-prod
