...
export function rafflesRouter(auth) {
  ...
  // POST /raffle/enter
  r.post("/raffle/enter", async (req, res) => {
    ...
    const { rows: r1 } = await pool.query("SELECT key FROM raffles WHERE key=$1 AND active=true", [key]);
    if (!r1.length) return res.status(400).json({ error: "raffle_inactive" });
    try {
  ...
  // GET /raffles/active
  r.get("/raffles/active", async (_req, res) => {
  ...
  r.get("/admin/raffle/entries", auth, async (req, res) => {
  ...
  // POST /admin/raffles
  r.post("/admin/raffles", auth, async (req, res) => {
  ...
  // PUT /admin/raffles/:key
  r.put("/admin/raffles/:key", auth, async (req, res) => {
