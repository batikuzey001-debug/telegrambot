...
export function messagesRouter(auth) {
  ...
  // GET /messages/:key
  r.get("/messages/:key", async (req, res) => {
  ...
  // GET /admin/messages
  r.get("/admin/messages", auth, async (_req, res) => {
    const { rows } = await pool.query(
      "SELECT key,content,image_url,file_id,updated_at FROM messages WHERE active=true ORDER BY key ASC"
    );
    res.json(rows);
  ...
  r.put("/admin/messages/:key", auth, async (req, res) => {
  ...
  // PUT /bot/messages/:key/file-id
  r.put("/bot/messages/:key/file-id", async (req, res) => {
