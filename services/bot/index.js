...
/** Send one message:
 *  - if image exists (file_id or image_url) -> send ONLY photo with caption
 *  - else -> send text
 */
...
const showPending = (ctx) =>
  ctx.reply("⏳ Başvurunuz inceleniyor. Onaylanınca üyelik ana sayfanız açılacak.", KB.PENDING_HOME);
...
bot.start(async (ctx) => {
  ...
  await sendMessageByKey(ctx, "welcome"); // will be single (photo OR text)
...
bot.action("g_signup", async (ctx) => {
  const kb = SIGNUP_URL
    ? Markup.inlineKeyboard([
        [Markup.button.url("📝 Kayıt Ol", SIGNUP_URL)],
        [Markup.button.callback("↩️ Geri", "go_guest")]
      ])
    : KB.GUEST_HOME;
...
bot.action("g_campaigns", (ctx) =>
  ctx.reply("📢 Kampanyalar (katılmak için üye olunmalı).", KB.GUEST_HOME)
);
...
bot.action("m_account", async (ctx) => {
  ...
  await ctx.reply("🧾 Hesap bilgileri yakında.", KB.MEMBER_HOME);
});
bot.action("m_free", async (ctx) => {
  if (!(await requireMember(ctx))) return;
  return sendMessageByKey(ctx, "member_free_events", KB.MEMBER_HOME);
});
...
bot.action("m_campaigns", async (ctx) => {
  ...
  try {
    ...
    if (!Array.isArray(data) || !data.length)
      return ctx.reply("ℹ️ Aktif kampanya yok.", KB.MEMBER_HOME);
    ...
    return ctx.reply("⚠️ Kampanyalar alınamadı.", KB.MEMBER_HOME);
  }
});
...
bot.action("m_raffle", async (ctx) => {
  ...
  try {
    ...
    return ctx.reply("⚠️ Çekiliş kaydı yapılamadı.", KB.MEMBER_HOME);
  }
});
...
httpApp.listen(process.env.PORT || 3001, () => {
  console.log("bot http invalidate ready");
});

/* ---------------- Launch ---------------- */
bot.launch({ dropPendingUpdates: true });
console.log("Bot: statüye göre yönlendirme; tek mesaj/tek görsel; invalidate aktif.");
