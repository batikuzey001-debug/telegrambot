...
export default function MessagesPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch("/admin/messages", { cache: "no-store" });
  ...
  async function save() {
    if (!editing) return;
    try {
      const res = await fetch(`/admin/messages/${encodeURIComponent(editing.key)}`, {
        method: "PUT",
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        (r.image_url || "").toLowerCase().includes(s)
    );
  }, [rows, q]);
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 8, margin: "12px 0" }}>
        <input
          placeholder="Ara: key / içerik / image url"
          value={q}
        ...
        <tbody>
          {filtered.map((r) => (
            <tr key={r.key}>
              ...
              <td style={td}>
                <button onClick={() => setEditing(r)}>Düzenle</button>
              </td>
            </tr>
          ))}
          {!filtered.length && (
      ...
      {editing && (
        <div style={panel}>
          ...
          {editing.image_url ? (
            <div style={{ margin: "8px 0" }}>
              <img
                ...
                style={{ maxWidth: "100%", maxHeight: 240, border: "1px solid #eee" }}
              />
            </div>
          ) : null}
          ...
          <p style={{ color: "#666", fontSize: 12, marginTop: 8 }}>
            Not: Yeni bir <b>Image URL</b> kaydederseniz API, ilgili mesajın <code>file_id</code> değerini sıfırlar.
          </p>
        </div>
      )}
    </div>
