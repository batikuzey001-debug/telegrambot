...
export default function MessagesPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        let body: any = null;
        try { body = await r.json(); } catch { body = { error: "json_parse_failed" }; }

        console.groupCollapsed("Backoffice › /api/messages debug");
        console.log("status:", r.status);
        console.log("ok:", r.ok);
        console.log("response body:", body);
        console.groupEnd();

        const arr: Msg[] = Array.isArray(body) ? body : [];
        setItems(arr);
        if (arr.length && !sel) {
          setSel(arr[0].key);
        ...
        setStatus(r.ok && arr.length ? "" : `Hata: ${body?.error || r.status}`);
      } catch (e: any) {
        console.error("fetch /api/messages failed:", e?.message || e);
        setStatus("API erişimi başarısız.");
        setItems([]);
      }
    })();
  ...
  useEffect(() => {
    ...
    if (m) { setContent(m.content || ""); setImageUrl(m.image_url || ""); setStatus(""); }
  }, [sel, items]);

  const filtered = useMemo(() => Array.isArray(items)
  ...
  async function save() {
    ...
    try {
      ...
      console.log("PUT /api/messages/[key] debug:", res.status, body);
      setStatus(res.ok ? "Kaydedildi. Bot cache temizlendi." : `Hata: ${res.status}`);
      if (res.ok) setItems(prev => prev.map(x => x.key === sel ? { ...x, content, image_url: imageUrl } : x));
    } catch (e: any) {
      console.error("save error:", e?.message || e);
      setStatus("Kayıt sırasında hata.");
    }
  }
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 16 }}>
      <aside style={{ borderRight: "1px solid #eee", paddingRight: 12 }}>
        <h2>Mesajlar</h2>
        <input placeholder="Ara (key)" value={q} onChange={e => setQ(e.target.value)} style={{ width: "100%", marginBottom: 8 }} />
